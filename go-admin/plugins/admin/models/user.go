...
func (t UserModel) WithMenus() UserModel {
	...
	for _, mid := range menuIdsModel {
		parentId, _ := strconv.ParseInt(string(mid["parent_id"].([]uint8)), 10, 64)
		menuId, _ := strconv.ParseInt(string(mid["menu_id"].([]uint8)), 10, 64)
		if parentId != 0 {
			for _, mid2 := range menuIdsModel {
				menuId2, _ := strconv.ParseInt(string(mid2["menu_id"].([]uint8)), 10, 64)
				if menuId2 == parentId {
					menuIds = append(menuIds, menuId2)
					break
				}
			}
		} else {
