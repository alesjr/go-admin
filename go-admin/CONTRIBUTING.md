...
See [manual](https://alesjr/go-admin/go-admin/projects/3) for more information.
...
For now, you need to add your fork as a remote on the original **\$GOPATH**/src/alesjr/go-admin/go-admin clone, so:
...
Notice: `go get` return `package alesjr/go-admin/go-admin: no Go files in /go/src/alesjr/go-admin/go-admin` is normal.
