...
See [manual](https://go-admin/go-admin/projects/3) for more information.
...
For now, you need to add your fork as a remote on the original **\$GOPATH**/src/go-admin/go-admin clone, so:

```bash

$ go get go-admin/go-admin
$ cd $GOPATH/src/go-admin/go-admin # GOPATH is $HOME/go by default.
...
Notice: `go get` return `package go-admin/go-admin: no Go files in /go/src/go-admin/go-admin` is normal.
...
* [Tasks](https://go-admin/go-admin/projects)
